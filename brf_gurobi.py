# brf_gurobi1.py — Class-Based BPR Traffic Assignment with EV Charging
# =================================================================
# Classes:
...
class BrfSolver:
    ...
    def _build_model(self):
        ...
        for od in self.od_path_dict:
            model.addConstr(
                gp.quicksum(var[('xop', od, p)] for p in self.od_path_dict[od])
        ...
        for arc_id in self.arc_path_dict:
            ...
            if t0 == 0:
                model.addConstr(var[('bpr', arc_id)] == 0)
            else:
                model.addConstr(
                    var[('bpr', arc_id)]
        ...
        # model.write('BPR_TAP.lp')

        self.model = model
        self.var = var
        model.update()
        print(f"  Vars: {model.NumVars}, Constraints: {model.NumConstrs + model.NumQConstrs}")
    ...
    def solve(self):
        ...
        self.status = status_map.get(
            self.model.Status, f'status_{self.model.Status}'
        )

        if self.model.SolCount > 0:
            print(f"  Status: {self.status}, Obj: {self.model.ObjVal:.6f}")
        else:
            print(f"  Status: {self.status}, no feasible solution")
    ...
    def extract_results(self):
        ...
        if model.SolCount <= 0:
            raise RuntimeError(
                f"BPR model has no feasible solution; status={getattr(self, 'status', model.Status)}"
            )

        xop = {}
        for od in self.od_path_dict:
            for p in self.od_path_dict[od]:
                xop[(od, p)] = var[('xop', od, p)].X

        ev_in = {}
        for ev_node in self.dh.IESS_nodes + self.dh.EVCS_nodes:
            vir_n = 'vir_' + str(ev_node)
            ev_in[ev_node] = var[('ev_in', vir_n)].X

        ya = {}
        bpr = {}
        for arc_id in self.arc_path_dict:
            ya[arc_id] = var[('ya', arc_id)].X
            bpr[arc_id] = var[('bpr', arc_id)].X
