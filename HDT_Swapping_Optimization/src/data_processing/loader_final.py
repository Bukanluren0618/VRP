...
# WRAPPED THE FUNCTIONS INTO A DATALOADER CLASS
class DataLoader:
    ...
    def load_all(self):
        ...
        self.road_network = G # Store the graph object
        ...
        pv_generation = {s: self.get_stochastic_pv_generation(time_steps, config.TIME_STEP_HOURS, config.PV_PEAK_POWER_KW,
                                                         config.PV_CLOUD_NOISE_LEVEL) for s in stations_info}
        ev_demand_timestep = {s: self.get_stochastic_ev_demand(time_steps, config.TIME_STEP_HOURS, config.EV_DEMAND_PEAK_KW,
