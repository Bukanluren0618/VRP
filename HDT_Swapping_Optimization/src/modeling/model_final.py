...
def create_operational_model(data, vehicle_ids, task_ids, config, fixed_route=None):
    ...
    # --- Big M Values ---
    # Ensure M_time is a sufficiently large positive number to prevent constraints from becoming ineffective
    M_time = (config.TIME_HORIZON_HOURS * 2) + 1
    ...
    # REMOVED: model.soc_deficit - We are now using a hard constraint for minimum SOC
    ...
    def objective_rule(m):
        ...
        # REMOVED: soc_deficit_penalty - No longer needed with the hard constraint
        unserved_task_penalty = 0
        if not fixed_route:
            # Correctly calculate the number of unserved tasks
            unserved_task_penalty = (len(m.TASKS) - sum(m.y[c, k] for c in m.CUSTOMERS for k in m.VEHICLES)) * config.UNASSIGNED_TASK_PENALTY
        return travel_cost + swap_cost + delay_penalty + unserved_task_penalty
    ...
    def departure_time_depot_rule(m, k):
        return m.departure_time[data['vehicles'][k]['depot_id'], k] == m.arrival_time[data['vehicles'][k]['depot_id'], k]
    ...
    def initial_weight_rule(m, k):
        # CORRECTED: More robust calculation of initial load
        initial_load = sum(customer_demands[c] * m.y[c, k] for c in m.CUSTOMERS if c in customer_demands)
    ...
    def delay_calculation_rule(m, t, k):
        customer = data['tasks'][t]['delivery_to']
        return m.delay_hours[t, k] >= m.arrival_time[customer, k] - data['tasks'][t]['due_time'] - M_time * (1 - m.y[customer, k])
    model.delay_calculation_constr = Constraint(model.TASKS, model.VEHICLES, rule=delay_calculation_rule)

    # CORRECTED: Changed from soft to hard constraint
    def min_soc_rule(m, k, n):
        # If vehicle k visits node n (y[n, k]=1), its arrival SOC must be >= min SOC.
        return m.soc_arrival[n, k] >= config.HDT_MIN_SOC_KWH - M_soc * (1 - m.y[n, k])
